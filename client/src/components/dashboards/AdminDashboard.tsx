...
export const AdminDashboard = () => {
  ...
  // Fetch lender applications
  const { data: lenderApplications = [] } = useQuery({
    queryKey: ['/api/admin/lender-applications'],
  });

  // Fetch all lenders for performance monitoring
          ...
          <Tabs defaultValue="pending" className="w-full">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline">{submission.amount}/month</Badge>
                        </div>
                        <p className="text-sm text-gray-600">
                          User ID: {submission.userId} • Payment Method: {submission.paymentMethod}
                        </p>
                      ...
                      <div className="flex gap-2">
                        {submission.paymentProofPath && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(`/uploads/payment-proofs/${submission.paymentProofPath}`, '_blank')}
                          >
            ...
            <TabsContent value="approved" className="space-y-4">
              {approvedSubmissions.map((submission: PaymentSubmission) => (
                <div key={submission.id} className="border rounded-lg p-4 bg-green-50 border-green-200">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <Badge className="capitalize bg-green-600">{submission.planName} Plan</Badge>
                        <Badge variant="outline">{submission.amount}/month</Badge>
                      </div>
                      <p className="text-sm text-gray-600">
                        User ID: {submission.userId} • Approved: {submission.reviewedAt ? new Date(submission.reviewedAt).toLocaleString() : 'N/A'}
                      </p>
                    </div>
                ...
                <div key={submission.id} className="border rounded-lg p-4 bg-red-50 border-red-200">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-gray-600">
                        User ID: {submission.userId} • Rejected: {submission.reviewedAt ? new Date(submission.reviewedAt).toLocaleString() : 'N/A'}
                      </p>
                      {submission.notes && (
                        <p className="text-sm text-red-600">Reason: {submission.notes}</p>
                      )}
                    </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-8 text-gray-500">
                ...
                <p>Payment submission review functionality available here.</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
